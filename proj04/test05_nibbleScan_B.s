...
main:
   ...
   #	lw      $s0,  0($t0)       # s0 =  start_regs[0]
	...
	# load value into s0.  If the student's register-save code is
	# working, this will be protected over the call.
	la      $s0, value
	lw      $s0, 0($s0)
...
main_LOOP:
	...
	# shift right.  Then if $s0 != 0, loop back, and run again.
	srl     $s0, $s0, 1
	bne     $s0, $zero, main_LOOP

	# run once more, but with 0.
	add     $a0, $s0, $zero
	jal     nibbleScan
