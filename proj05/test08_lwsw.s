...
PROGRAM:
    .word 0x20080010        #addi $zero $t0 0x10
    .word 0x01084820        #add  $t1 $t0 $t0 
    .word 0x01285020        #add  $t2 $t1 $t0 
    .word 0x01295820        #add  $t3 $t1 $t1

    .word 0x20140008        #addi $s4, $zero, 8
    #.word 0xae89ffff        #sw   $t1, -1($s4) // Use this line to make sure you return 2 on misaligned pointers
    .word 0xae89fffc        #sw   $t1, -4($s4)
    .word 0xae8a0000        #sw   $t2, 0($s4)
    .word 0xae8b0004        #sw   $t3, 4($s4)
    .word 0xAE8803E8        #sw   $t0, 1000($s4) // If you set the memory to be much lower this line should fail

    #.word 0x8e89ffff        #lw   $t1, -1($s4) 
    .word 0x8e8dfffc        #lw   $t5, -4($s4)
    .word 0x8e8e0000        #lw   $t6, 0($s4)
    .word 0x8e8f0004        #lw   $t7, 4($s4)
    .word 0x8e9103e8        #lw   $s1, 1000($s4)
...
MEMORY:
    .space  16384              # 16 KB // modify this to make sure your program checks you're within memsize
END_MEMORY:
